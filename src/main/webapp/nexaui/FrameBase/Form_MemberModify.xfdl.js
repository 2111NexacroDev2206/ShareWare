(function()
{
    return function()
    {
        ...
        {
            this.set_name("Form_MemberModify");
            ...
            obj = new Button("btn_remove","1000","610","140","50",null,null,null,null,null,null,this);
            obj.set_taborder("1");
            obj.set_text("삭제");
            obj.set_font("normal 13pt/normal \"Arial\"");
            this.addChild(obj.name, obj);

            obj = new Button("btn_modify","830","610","140","50",null,null,null,null,null,null,this);
            obj.set_taborder("2");
            obj.set_text("수정");
            obj.set_font("normal 13pt/normal \"Arial\"");
            this.addChild(obj.name, obj);
            ...
            obj = new BindItem("item2","Div00.form.divName","value","ds_member","divCode");
            this.addChild(obj.name, obj);
            obj.bind();

            obj = new BindItem("item3","Div00.form.rankName","value","ds_member","rankCode");
            this.addChild(obj.name, obj);
            obj.bind();
            ...
            obj = new BindItem("item15","Div00.form.ImageViewer00","image","ds_member","photo");
            this.addChild(obj.name, obj);
            obj.bind();
        ...
        // User Script
        this.registerScript("Form_MemberModify.xfdl", function() {
        var objApp = nexacro.getApplication();
        var memberNum = objApp.getVariable("memberNum");

        // 화면 로드 시
        this.Form_MemberModify_onload = function(obj,e)
        {
        	this.transaction(
        			"member_detail" // 1. ID
        			, "SW::admin/member/detail.sw" // 2. URL
        			, "" // 3. InDs : F->S jsp(I, U, D)
        			, "ds_member=out_member" // 4. OutDs : S->F jsp(SELECT) - 스페이스는 구분자라서 = 옆에 스페이스 없이 써야 함
        			, "memberNum=" + memberNum // 5. InVar : F->S(var)
        ...
        this.fn_callback_tran = function(id, nErrorCode, sErrorMsg)
        {
        	...
        	}else if(id=="member_modify")
        	{
        		if(nErrorCode < 0)
        		{
        			this.alert("사원 정보 수정 실패 : " + sErrorMsg);
        			return;
        		}else {
        			alert("사원 정보 수정 완료");
        			this.go("FrameBase::Form_MemberList.xfdl");
        		}
        	}else if(id=="member_remove")
        	{
        		if(nErrorCode < 0)
        		{
        			this.alert("사원 삭제 실패 : " + sErrorMsg);
        			return;
        		}else {
        			this.go("FrameBase::Form_MemberList.xfdl");
        		}
        ...
        {
        	var rtn = this.confirm("삭제하시겠습니까?", "사원 삭제");
        	if(rtn == true) {
        		this.transaction(
        			"member_remove" // 1. ID
        			, "SW::admin/member/remove.sw" // 2. URL
        			, "" // 3. InDs : F->S jsp(I, U, D)
        ...
        this.btn_modify_onclick = function(obj,e)
        {
        	this.transaction(
        			"member_modify" // 1. ID
        			, "SW::admin/member/modify.sw" // 2. URL
        ...
        {
            this.addEventHandler("onload",this.Form_MemberModify_onload,this);
        ...
        this.loadIncludeScript("Form_MemberModify.xfdl");
