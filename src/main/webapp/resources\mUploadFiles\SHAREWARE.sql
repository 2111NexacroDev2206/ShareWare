...
SELECT * FROM          
			...
			JOIN MAIL_FILE USING(MAIL_NO))
            JOIN MAIL_REC USING(MAIL_NO)
			WHERE (MAIL_RECEIVER = 'admin' OR MAIL_SENDER = 'admin') AND MAIL_FILE_NAME IS NOT NULL;    
...
SELECT * FROM MAIL JOIN MAIL_REC USING(MAIL_NO)

				WHERE MAIL_RECEIVER LIKE '%' ||'1' || '%'
				OR MAIL_SUBJECT LIKE '%' ||'1'|| '%'
				OR MAIL_CONTENT LIKE '%' || '1' || '%'
